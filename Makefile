...
SRC = $(TARGET).c cli.c serial.c timer.c rht03.c sht1x.c rht.c ossd_i2c.c ns741.c lib/twimaster.c lib/uart.c
SRCPP = 
...
# twimaster.c can be replaced with i2cmaster.S
#ASRC = i2cmaster.S
...
CDEFS = -DF_CPU=$(F_CPU)UL -D_DEBUG=1

# Peter's' Fleury UART library parameters
...
OBJ = $(SRC:.c=.o) $(SRCPP:.cpp=.o) $(ASRC:.S=.o) 

# Define all listing files.
LST = $(SRC:.c=.lst) $(SRCPP:.cpp=.lst) $(ASRC:.S=.lst) 
...
size:
	@$(SIZE) --mcu=$(MCU) -C $(TARGET).elf | egrep 'Program|Data|EEPROM'
...
clean clean_list program debug gdb-config reset size
